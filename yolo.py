...
class YOLO(object):
    _defaults = {
        ...
        "model_path"        : 'model_data/ep100-loss12.678-val_loss13.127.h5',
    ...
    def detect_image(self, image):
        ...
        small_pic=[]
        lablesed=""
        labelses=[0]*self.num_classes
        for i, c in list(enumerate(out_classes)):
            predicted_class = self.class_names[int(c)]
            labelses[int(c)]+=1
            box             = out_boxes[i]
            score           = out_scores[i]
            # if predicted_class not in lablesed:
            #     lablesed= predicted_class + " ; "+ lablesed
        ...
        for i in range(self.num_classes):
            if labelses[i]!=0:
                lablesed = self.class_names[i] + str(labelses[i]) +"个 ; " + lablesed
        return image,lablesed
